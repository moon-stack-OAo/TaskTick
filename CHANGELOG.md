...
- 可选 C# FlaUI Agent（`tools/wecom-agent`）：Release 附件 `wecom-agent-windows-x64.zip`（自包含）；设置页可下载、选路径并检测；**不**打入安装包
...
- `npm run agent:stage`：发布自包含 Agent 供 CI 打 zip；安装包不依赖 externalBin
...
- FlaUI Agent 需从 Release 下载 zip；未配置时可用键鼠回退（默认开启）
